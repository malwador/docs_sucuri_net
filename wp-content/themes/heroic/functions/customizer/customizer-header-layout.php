<?php
function heroic_header_layout_customizer($wp_customize) {
     ...
     */
    class heroic_Image_Radio_Button_Custom_Control extends WP_Customize_Control {
    ...
    $heroic_current_options = wp_parse_args(get_option('quality_pro_options', array()), quality_theme_data_setup());
       ...
       /* Header Layout section */
        ...
        'title' => esc_html__('Header Setting', 'heroic'),
         ));
        
        $wp_customize->add_section( 'header_sticky_layout_setting' , array(
            'title'      => esc_html__('Header Layout', 'heroic'),
        ...
        if(get_option('quality_user', 'new')=='old' || $heroic_current_options['text_title'] !='' || $heroic_current_options['upload_image_logo']!='' || $heroic_current_options['webrit_custom_css']=='nomorenow' ){
            ...
            $wp_customize->add_control(new heroic_Image_Radio_Button_Custom_Control($wp_customize, 'quality_pro_options[header_sticky_layout_setting]',
                    array(
                'label' => esc_html__('Header Layout Setting', 'heroic'),
                'section' => 'header_sticky_layout_setting',
                'choices' => array(
                    'default' => array(
                        'image' => get_stylesheet_directory_uri() . '/images/heroic-header-default.png',
                        'name' => esc_html__('Standard Layout', 'heroic')
                    ),
                    'sticky' => array(
                        'image' => get_stylesheet_directory_uri() . '/images/heroic-header-overlay.png',
                        'name' => esc_html__('Overlay Layout', 'heroic')
                    )
                )
                    )
    ));
...
add_action('customize_register', 'heroic_header_layout_customizer');

//radio box sanitization function
function heroic_sanitize_radio($input, $setting) {
