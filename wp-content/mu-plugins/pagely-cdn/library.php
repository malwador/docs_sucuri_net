...
class Pagely_CDNConfig
{
    ...
    public function load()
    {
        ...
        // if FSA is active, we want to disable all other functionality of the CDN
        if (!empty($config->fsa->status) && in_array($config->fsa->status, ['active', 'active-modifying'])) {
            $this->fsa_enabled = 1;
            // force disable CDN when FSA is active as we don't want any of the rewrite or other functionality
            $this->enabled = 0;
        }
    }
...
class Pagely_CDNLinksRewriter
{
    ...
    {
        if (defined('PAGELY_CDN_EXCLUDE_PATHS') && is_array(PAGELY_CDN_EXCLUDE_PATHS)) {
            if (isset($_SERVER['REQUEST_URI']) && in_array($_SERVER['REQUEST_URI'], PAGELY_CDN_EXCLUDE_PATHS)) {
                return $content;
            }
        }
    ...
    public function setBlogUrlDirFromHomeUrl($homeUrl, $contentUrl = false)
    {
        ...
        $this->blogDir = apply_filters('pagely_blogdir_filter', $this->blogDir);
